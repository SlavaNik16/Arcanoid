...
namespace ArcanoidNV
{
    public partial class Form1 : Form
    {
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            if (ball.X > ClientRectangle.Width - 100)
            {
                dx = -dx;
            }

            if (ball.Y > ClientRectangle.Height - 60)
            {
        ...
        private void Form1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (e.X < ClientRectangle.Width - player.width /2)
            {
                x = e.X;
            }
            else
            {
                x = ClientRectangle.Width - player.width / 2;
            }
            ...
            for (int i = 1; i <= 6; i++)
            {
                ...
                if (Nrnd > 0)
                {
                    for (int j = 1; j <= 8; j++)
                    {
                        walls.Add(new Wall
                        {
                            X = j * (wall.width + 4) - wall.width + 30,
