...
namespace Arcanoid
{
    public partial class Form1 : Form
    {
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            if ((ball.X >= player.X + 60 && ball.X <= player.X + 110) && (ball.Y >= player.Y - 20))
            {
                ball.speedX = 0;
                ball.speedY = -ball.delta;
            }
            else if ((ball.X >= player.X && ball.X < player.X + 60) && (ball.Y >= player.Y - 20))
            {
                ball.speedX = -ball.delta - 20;
                ball.speedY = -ball.delta;
            }
            else if ((ball.X > player.X + 110 && ball.X < player.X + 190) && (ball.Y >= player.Y - 20))
            {
                ball.speedX = ball.delta + 20;
                ball.speedY = -ball.delta;
            }
